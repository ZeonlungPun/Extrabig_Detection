...
def slice_tif(im_path, out_dir, out_name,
...
def slice_to_png(tif_path, out_path, out_space,
    ...
    #获取输入路径中的全部tif文件
    im_list = [z for z in os.listdir(tif_path) if z.endswith('tif')]

    #拼完整输出路径出来
    out_folder = os.path.join(out_path, out_space)
    out_txt = out_folder + '/' + 'slice.txt'
    #不存在则创建，存在则给出提示信息
    if not os.path.exists(out_folder):
        ...
        for i, im_name in enumerate(im_list):
            ...
            slice_tif(im_path=im_path, out_dir=out_folder, out_name=out_name,
        ...
        for f in sorted([z for z in os.listdir(out_folder) if z.endswith(out_ext)]):
            ...
            df_tmp.to_csv(out_txt, header=False, index=False)
    else:
        print("输出文件加已存在，路径为:", out_folder)

    print("进程结束，所用时间为:", time.time()-t0, "秒")


if __name__ == "__main__":
    slice_to_png(tif_path='/home/kingargroo/m4', out_path='/home/kingargroo/out', out_space='M4')